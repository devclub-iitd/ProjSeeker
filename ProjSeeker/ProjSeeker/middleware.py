...
PUBLIC_KEY = "ProjSeeker/public.pem"
...
DEFAULT_ROLES = [
    os.environ.get("DEFAULT_CLIENT_ROLE", "external_user"),
    os.environ.get("IITD_USER_ROLE", "iitd_user"),
]

UNAUTHORIZED_HANDLER = lambda request: HttpResponse(
    "Alas You are out of scope! Go get some more permissions dude", status=401
)
from portal.views import make_new_user

MAKE_USER_CALLBACK = make_new_user
...
class SSOMiddleware:
    ...
    def __call__(self, request):
        ...
        if token is not None:
            try:
                ...
                if float(decoded["exp"]) - time.time() < MAX_TTL_ALLOWED:
                    decoded["user"] = self.refresh(
                        request=request, token={SSO_TOKEN: token}
                    )
        ...
        if self.cookies is not None:
            response._headers["set-cookie1"] = (
                "Set-Cookie",
                self.cookies.split("\n")[0],
            )
            try:
                response._headers["set-cookie2"] = (
                    "Set-Cookie",
                    self.cookies.split("\n")[1],
                )
            except:
    ...
    def assign_user(self, request, user_payload):
        ...
        try:
            ...
            user = USER_MODEL.objects.create_user(
                email=user_payload["email"], username=user_payload["username"]
            )
            MAKE_USER_CALLBACK(request, user, user_payload)
    ...
    def redirect(self, request):
        ...
        original_url = original_url.replace("vm1-internal", "devclub.in")
        original_url = original_url.replace("vm2-internal", "devclub.in")

        return redirect(AUTH_URL + f"/?{QUERY_PARAM}={original_url}")
