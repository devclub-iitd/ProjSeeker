...
class Departments(models.TextChoices):
    ...
    CE = "civil", _("Civil Engineering")
    BB = "dbeb", _("Biotech Engineering")
    TT = "textile", _("Textile Engineering")
    PH = "physics", _("Engineering Physics")
    MT = "maths", _("Mathematics")
    AM = "am", _("Applied Mechanics")
    MS = "mse", _("Material Science")
    CSC = "csc", _("Computer Service Centre")
    ESE = "dese", _("Centre for Energy Studies")
    DMS = "dms", _("Department of Management Studies")
    CBME = "cbme", _("Centre for Biomedical Engineering")
    RDAT = "rdat", _("Centre For Rural Development And Technology")
    CES = "ces", _("Centre for Energy Studies")
    DESIGN = "design", _("Department of Design")
    HSS = "hss", _("Humanities and Social Sciences")
    CARE = "care", _("Centre for Applied Research in Electronics")
    CART = "cart", _("Centre for Automotive Research and Tribology")
    SENSE = "sense", _(
...
def upload_pic(s, _):
    return upload_handler(s, "pic.jpg")


def upload_cv(s, _):
    return upload_handler(s, "cv.pdf")


def upload_transcript(s, _):
    return upload_handler(s, "transcript.pdf")


def upload_noc(s, _):
    return upload_handler(s, "noc.pdf")


class Student(models.Model):
    user = models.ForeignKey(
        User, verbose_name=_("Auth User"), on_delete=models.CASCADE
    )
    ...
    dept = models.CharField(
        _("department"), max_length=50, choices=Departments.choices, null=True
    )
    cv = models.FileField(
        _("Resume"), upload_to=upload_cv, storage=OverwriteStorage(), null=True
    )
    transcript = models.FileField(
        _("Transcripts"),
        upload_to=upload_transcript,
        storage=OverwriteStorage(),
        null=True,
    )
    pic = models.FileField(
        _("Profile Pic"),
        upload_to=upload_pic,
        null=True,
        storage=OverwriteStorage(),
        blank=True,
    )
...
class Project(models.Model):
    ...
    cpi = models.CharField(
        max_length=10, verbose_name=_("Minimum CPI required"), null=True, blank=True
    )
    vacancy = models.PositiveSmallIntegerField()
    min_year = models.CharField(
        max_length=10,
        verbose_name=_("Minimum years of study completed"),
        null=True,
        blank=True,
    )
    ...
    depts = MultiSelectField(choices=Departments.choices, null=True, blank=True)
...
class Application(models.Model):
    ...
    project = models.ForeignKey(
        "Project", verbose_name=_("Project applied to"), on_delete=models.CASCADE
    )
