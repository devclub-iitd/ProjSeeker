...
class ProjectViewSet(ModelViewSet):
    ...
    def find_projects(self, request):
        return render(
            ...
            context={
                "depts": Departments.choices,
                "degrees": Degree.choices,
                "types": Project.ProjectType.choices,
                "durations": Project.Duration.choices,
                "depts": Departments.choices,
            },
        )
    ...
    def applied_projects(self, request):
        return render(
            request=request,
            template_name="applied-projects.html",
            context={
                "is_student": isStudent(request.user),
                "is_prof": isProf(request.user),
            },
    ...
    def apply_for_project(self, request, pk=None):
        ...
        return render(
            ...
            context={"project": serializer.data, "isApplied": False},
        )
    ...
    def edit(self, request, pk=None):
        ...
        interest_text = ", ".join(
            [it["research_field"] for it in serializer.data["tags"]]
        )
...
class BookmarkViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if Bookmark.objects.filter(
            user__id=request.user.id, project__id=project_id
        ).exists():
...
def get_uploaded_file(request, pk, file_name):
    user = request.user
    if user.id != pk or file_name not in [
        "cv.pdf",
        "transcript.pdf",
        "pic.jpg",
        "noc.pdf",
...
class ProfileViewSet(ModelViewSet):
    ...
    def check_object_permissions(self, request, obj):
        if obj.user.id != request.user.id:
            self.permission_denied(request, message="Unauthorized User", code=403)
    ...
    def profile(self, request):
        ...
        interest_text = ", ".join(
            [it["research_field"] for it in serializer.data["interests"]]
        )
        return render(
            request,
            ...
            context={
                ...
                "is_student_render": isStudent(user),
                "is_prof_render": isProf(user),
            },
        )
...
class InterestViewSet(ModelViewSet):
    ...
    filter_backends = [filters.DjangoFilterBackend]
    filter_fields = {"research_field": ["startswith"]}
...
class ApplicationViewSet(ModelViewSet):
    ...
    def check_object_permissions(self, request, obj):
        if (
            obj.student.user.id != request.user.id
            and obj.project.prof.user.id != request.user.id
        ):
            self.permission_denied(request, message="Forbidden", code=403)
        if (
            request.method != "GET"
            and obj.project.deadline_passed()
            and isStudent(request.user)
        ):
            self.permission_denied(request, message="Deadline Passed!", code=400)
        return super().check_object_permissions(request, obj)
    ...
    def view_received_applications(self, request):
        return render(
            request=request,
            template_name="application-card.html",
            context={
                "is_prof": isProf(request.user),
                "is_student": isStudent(request.user),
            },
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        project_data = ProjectSerializer(
            Project.objects.get(id=instance.project.id), many=False
        ).data
        serializer = self.get_serializer(instance, many=False)
        student_name = (
            f"{instance.student.user.first_name} {instance.student.user.last_name}"
        )
        ...
        return render(
            ...
            context={
                ...
                "student_user_id": instance.student.user.id,
    ...
    def update(self, request, *args, **kwargs):
        ...
        if (
            "status" in request.data.keys()
            and request.data["status"] != Status.rejected
        ):
            request.data["remark"] = "-"
        return super().update(request, *args, **kwargs)
